...
deploy to a local {kube} cluster.You will then create a Kubernetes Service which load balance between containers and
verify that you can share data between Microservices.

Caching plays an important role in high performing microservices. Hazelcast keeps frequently accessed data in-memory
so you can store your database queries,lookup data or http sessions for fast access. Hazelcast is elastic so it scale out/in 
easily on commodity hardware. Data is automatically distributed among new hazelcast cluster members.
...
[Hazelcast](https://hazelcast.org/) is an open source In-Memory Data Grid (IMDG). It provides elastically scalable distributed In-Memory computing,
...
Hazelcast supports multiple cloud-native options when it comes to deployment.

- embedded: Hazelcast starts with each of your microservice and joins to the existing Hazelcast Cluster 
- client/server: Microservices can connect to a remote Hazelcast Cluster 
- Hazelcast Cloud: Microservices can can keep their data in serverless manner with [Hazelcast Cloud](https://cloud.hazelcast.com/)

This guide uses embedded option.
...
----
#Replace `Application.java` class.#
src/main/java/io/openliberty/guides/hazelcast/Application.java
----
...
if you check the source code of [hotspot file=1]`CommandController.java`, you would notice that [hotspot=21 file=1]`ConcurrentHashMap` is used
for backing key value store. In order to make this cache distributed, replace [hotspot file=1]`CommandController.java`
with Hazelcast version.

[role="code_command hotspot file=1", subs="quotes"]
----
#Replace `CommandController.java` class.#
src/main/java/io/openliberty/guides/hazelcast/CommandController.java
----
...
You would notice that getting hazelcast member is [hotspot=18 file=1]`Autowired` by Spring Boot and made available to the [hotspot=13 file=1]`CommandController`
and there is no change in [hotspot=25 file=1]`/put` and [hotspot=31 file=1]`/get` endpoints.
...
To deploy the `hazelcast-caching` microservice, first create the [hotspot file=0]`kubernetes.yaml` file in the `start` directory:
...
By default, we create 2 replicas of [hotspot=12 file=0]`hazelcast-caching` microservice behind the [hotspot=9 file=0]`hazelcast-caching-service` which forwards
requests to one of the pods available in the kubernetes cluster.

[hotspot=26 file=0]`MY_POD_NAME` is an environment variable made available to the pods so that each microservice knows which pod they are in.
...
Run the following commands to deploy the resources as defined in [hotspot file=0]`kubernetes.yaml` and [hotspot file=1]`rbac.yaml` in the specified order:
...
----
include::finish/src/test/java/it/io/openliberty/guides/hazelcast/HazelcastCachingIT.java[tags=**;!copyright]
----

The [hotspot=41-65]`testHazelcastCache` test makes sure that the [hotspot=48]`/put` endpoint is handled by one pod
and [hotspot=57]`/get` methods returns the same data from the other kubernetes pod.

It first puts a key/value pair to `hazelcast-caching` microservice and keeps podname in the `firstpod`
variable. In the second part, tests submits multiple [hotspot=60]`/get` requests until to see that podname is different then the pod
which initially handled [hotspot=62]`/put` request.
...
| [hotspot=34]`cluster.ip`            | IP or hostname for your cluster, `{minikube-ip}` by default, which is appropriate when using Minikube.
| [hotspot=35]`cluster.port`          | Port of the {kube} Service wrapping the `hazelcast-caching` pods, `hazelcast-caching-service` by default.
|===
